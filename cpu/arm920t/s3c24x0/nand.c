...
static void s3c2440_hwcontrol(struct mtd_info *mtd, int cmd)
{
	...
	switch (cmd) {
	case NAND_CTL_SETNCE:	//1
		//NFCONF &= ~S3C2410_NFCONF_nFCE;
		s3c2440nand->NFCONT &= ~(1<<1);
		DEBUGN("NFCONT=0x%08x\n", s3c2440nand->NFCONT);
		//for(i=0; i<10; i++); 
		break;
	case NAND_CTL_CLRNCE:	//2
		//NFCONF |= S3C2410_NFCONF_nFCE;
		s3c2440nand->NFCONT |= (1<<1);
		DEBUGN("NFCONT=0x%08x\n", s3c2440nand->NFCONT);
		break;
	case NAND_CTL_SETALE:	//5
		//chip->IO_ADDR_W = NF_BASE + 0x8;
		chip->IO_ADDR_W = &(s3c2440nand->NFADDR);
		DEBUGN("SETALE=0x%x\n",chip->IO_ADDR_W);
		break;
	case NAND_CTL_SETCLE:	//3
		//chip->IO_ADDR_W = NF_BASE + 0x4;
		chip->IO_ADDR_W = &(s3c2440nand->NFCMD);
		DEBUGN("SETCLE=0x%x\n",chip->IO_ADDR_W);
		break;
	default:
		//chip->IO_ADDR_W = NF_BASE + 0xc;
		chip->IO_ADDR_W = &(s3c2440nand->NFDATA);
		DEBUGN("DEF=0x%x\n",chip->IO_ADDR_W);
		DEBUGN("DEF=0x%x\n",chip->IO_ADDR_R);
		break;
	}
...
#define TWRPH1  0
	...
	/* initialize nand_chip data structure */
	nand->IO_ADDR_R = nand->IO_ADDR_W = &(s3c2440nand->NFDATA);
	...
	nand->correct_data = s3c2440_nand_correct_data;
	nand->eccmode = NAND_ECC_HW3_512;
#else
	nand->eccmode = NAND_ECC_SOFT;
#endif
